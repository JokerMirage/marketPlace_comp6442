 ...
 */
package com.market.sever.service;

import com.market.sever.config.BSTree;
import com.market.sever.config.Node;
...
public class Sever {
            ...
            while (keepGoing) {
                ...
                if (tree.find(t.userId) == null) {
                    ...
                    if (tree.find(t.userId).thread.isAlive()) {
                        LOG.warn("Existing thread is running");
                    }
                }
            }
    ...
    private synchronized void sendMessage(String message, int destination) throws IOException {
        ...
        if (!ct.writeMsg(messageLf)) {
            LOG.warn("Sever received the message but fail to send");
        }
        LOG.info("Message was sent out");
